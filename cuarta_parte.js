...
function move(dir){
    ...
    if(unoccupied(current.type,current.x,current.y,dir)){
        current.x=x;
        current.y=y;
        invalidate();
        return true;
...
function drop(){
    if(!move(DIR.DOWN)){
        ...
        clearActions();
        if(occupied(current.type,current.x,current.y)){
            lose();
        }
    }
};
...
function drawNext(){
    if(invalid.next){
        var padding=dx*(nu-next.type.size)/2;
        uctx.save();
